...
def analisis_log(entornos,n_A,n_B,kappa_0_to_1,kappa_1_to_0,k_A,k_B,pi_A_to_B,pi_B_to_A, posicion_analisis_1):
    ...
    n, bins = np.histogram(n_analizar_1,bins=8,density=True)
    ...
    fokker_planck=density_probability(x_range_2,math.log(10),0.000072,0.2,t1)
    super_nor=survival_function(t1,math.log(10),0.000072,0.2) 
...
def media_varianza(entornos,n_A,n_B,kappa_0_to_1,kappa_1_to_0,k_A,k_B,pi_A_to_B,pi_B_to_A):
    ...
    matriz10=np.zeros((10000, len(tiempo)))
    matriz40=np.zeros((10000, len(tiempo)))
...
def trayectorias_log(entornos,n_A,n_B,kappa_0_to_1,kappa_1_to_0,k_A,k_B,pi_A_to_B,pi_B_to_A):
    ...
    for j in range(len(n_B_1)):
        if entornos[j]==0:
            axis[0,0].plot(n_A_1[j],n_B_1[j],'.', color='blue',markersize=2)
        else:
            axis[0,0].plot(n_A_1[j],n_B_1[j],'.', color='red',markersize=2)
    ...
    legend_elements_1 = [Line2D([0], [0], marker='.', color='w', label='Ambiente 0',markerfacecolor='b', markersize=10),
                   Line2D([0], [0], marker='.', color='w', label='Ambiente 1',markerfacecolor='r', markersize=10),
    ...
    for j in range(len(n_B_1)):
        print(entornos[j])
        if entornos[j]==0:
            axis[0,1].plot(tiempo[j],n_B_1[j],'.', color='blue',markersize=2)
        else:
            axis[0,1].plot(tiempo[j],n_B_1[j],'.', color='red',markersize=2)
    #axis[0,1].set_title('Trajectory of ln $n_B(t)$'); 
    axis[0,1].set_xlabel('Tiempo',fontsize=12)
    ...
    axis[0,1].plot(tiempo,n_A_1+math.log((1/phi_1)-1),'g')
    axis[0,1].plot(tiempo,n_A_1+math.log((1/phi_2)-1),'k')
    axis[0,0].get_shared_y_axes().join(axis[0,0], axis[0,1])
    #Temporal evolution of ln n_A.
    for j in range(len(n_A_1)):
        if entornos[j]==0:
            axis[1,0].plot(tiempo[j],n_A_1[j],'.', color='blue',markersize=2)
        else:
            axis[1,0].plot(tiempo[j],n_A_1[j],'.', color='red',markersize=2)
    axis[1,0].set_ylabel('Ln $n_A(t)$',fontsize=12); axis[1,0].set_xlabel('Tiempo',fontsize=12); 
    ...
    axis[1,0].plot(tiempo,n_B_1-math.log((1/phi_1)-1),'g')
    axis[1,0].plot(tiempo,n_B_1-math.log((1/phi_2)-1),'k')
    ...
    axis[1,1].set_xlabel('Tiempo',fontsize=12) 
...
'''
fokker_planck,y_fit_normalizada,n_analizar_1,x_range,x_range_2,supervivencia,super_nor,t1,mu_fit,sigma_fit,n,bins=analisis_log(entornos, n_A, n_B, kappa_0_to_1, kappa_1_to_0, k_A, k_B, pi_A_to_B, pi_B_to_A,18000)

guardar_datos((fokker_planck, y_fit_normalizada, n_analizar_1, x_range, x_range_2, supervivencia, super_nor, t1, mu_fit, sigma_fit,n,bins), "datos_guardados.pkl")
'''
'''
...
mu=0.210
sigma=0.91
#Survial function and its representation.
